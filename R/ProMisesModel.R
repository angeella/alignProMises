...
#' @return \code{vMFPmodel} returns a list with four components:
...
ProMisesModel <- function(data, maxIt=10, t =.001, k = 0, Q = NULL, ref_ds = NULL, scaling = T, reflection= T, subj= F, centered = T, kCalibrate = FALSE, D = NULL, p = 0.01){
  ...
  while(dist[count] > t & count < maxIt){
    ...
    out <-foreach(i = c(1:nsubj)) %dopar% {
      if(subj){
        ...
        GPASub(X[,,i], Q[,,i], k, kQ, ref_ds, scaling, reflection)
      }else{
        #  GPASub(X[,,i], Q, k, ref_ds, scaling, reflection) 
        GPASub(X[,,i], Q, k, kQ, ref_ds, scaling, reflection) 
      }
